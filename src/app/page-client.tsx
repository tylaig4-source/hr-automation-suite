...
// Função para converter planos do banco para formato da landing
function transformPlansForLanding(dbPlans: PlanFromDB[]): LandingPlan[] {
  return dbPlans.map((plan) => {
    ...
    return {
      ...
      cta: plan.isEnterprise ? "Falar com consultor" : "Ver opções de pagamento",
      color: plan.isEnterprise ? "#8b5cf6" : "#ff00ff",
      icon: plan.isEnterprise ? "Building2" : "Zap",
      badge: plan.isPopular ? "Mais Popular" : null,
    };
  });
}
...
export default function HomePage({ plans: dbPlans = [] }: HomePageProps) {
  ...
  const yearlyPrice = professionalPlan?.price ? parseFloat(professionalPlan.price) : 497;
      ...
      <div className="relative z-10">
        ...
        >
          <div className="container mx-auto px-4">
                ...
                >
                  ...
                  {billingCycle === "YEARLY" && professionalPlan && professionalPlan.monthlyPrice && professionalPlan.price && (
                    <span className="ml-2 px-2 py-0.5 rounded-full bg-green-500 text-white text-xs font-bold">
                      -R$ {savings.toLocaleString("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                    </span>
                  )}
                </button>
                <button
            ...
            {billingCycle === "YEARLY" && professionalPlan && professionalPlan.monthlyPrice && professionalPlan.price && savings > 0 && (
              <div className={`flex justify-center mb-8 transition-all duration-1000 delay-100 ${pricingReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <div className="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-green-500/30 bg-green-500/10 text-green-400 animate-pulse">
                  ...
                  <span className="font-semibold">Economize R$ {savings.toLocaleString("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}/ano no plano anual</span>
                </div>
              </div>
            )}
                          ...
                          <div>
                            ...
                            {billingCycle === "YEARLY" && plan.price && plan.monthlyPrice ? (
                              <div className="mt-2">
                                ...
                                {(() => {
                                  ...
                                  const yearly = parseFloat(plan.price!);
                                  const annualSavings = (monthly - yearly) * 12;
                                  if (annualSavings > 0) {
                                    return (
                                      <div className="mt-1 inline-flex items-center gap-1 px-3 py-1 rounded-full bg-green-500/20 border border-green-500/30">
                                        <span className="text-xs font-bold text-green-400">
                                          Economia de R$ {annualSavings.toLocaleString("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}/ano
                                        </span>
                                      </div>
                                    );
                                  }
                            ...
                            {billingCycle === "MONTHLY" && plan.price && (
                              <div className="mt-2 line-through text-gray-600 text-sm">
                                <span className="text-gray-500">Anual: R$ {parseFloat(plan.price).toLocaleString("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}/mês</span>
                              </div>
                            )}
                          </div>
                        )}
                  ...
                  <tbody>
                    <tr className="border-b border-white/5 bg-green-500/5">
                      ...
                      <td className="text-center p-4 text-white font-semibold">
                        R$ {yearlyPrice.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </td>
                      <td className="text-center p-4 text-white">
                        R$ {yearlyTotal.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </td>
                    ...
                    <tr className="border-b border-white/5">
                      ...
                      <td className="text-center p-4 text-white">
                        R$ {monthlyPrice.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </td>
